//===============
// NEXIO TORII PARSING ENGINE (UNIFIED, FIXED FOR SEQUELS & S1-SLOP PREVENTION)
// Parses chaotic fan-sub titles to definitively verify episode targets.
//===============

//===============
// SANITIZE FILENAME
// Strips codecs, groups, and irrelevant metadata tags out of the title
// so that numeric extraction doesn't accidentally catch a "x264" as episode 264.
//===============
...
//===============
// NORMALIZE TITLE
// Translates Roman Numerals and Japanese spacing to ensure English verification works.
//===============
...
//===============
// IS WRONG SEASON
// Strict verification mechanism to drop torrents meant for Season 3 
// if Stremio is asking for Season 1. Prevents "Slop".
//===============
...
function extractEpisodeNumber(filename, expectedSeason = 1) {
    ...
    // Most aggressive and strict regex for standard EP notations (Ep 05, S01E05)
    ...
    // Checks for isolated numbers right at the end of the title string
...
//===============
// GET BATCH RANGE
// Identifies if a string is a range like "01-12" or "01 to 24" so we 
// know this file covers multiple episodes.
//===============
...
// Deep inspection of RD / Torbox generated file structures.
// Identifies the exact MKV or MP4 belonging to the requested episode within
// a large directory tree, filtering out metadata extras like trailers or making-ofs.
//===============
function selectBestVideoFile(files, requestedEp, expectedSeason = 1, isMovie = false) {
    ...
    // SABOTAGE REMOVED: isMovie flag handles Trailers safely.
    if (epNum > 1) {
        isMovie = false;
    }
    ...
    if (!isMovie) {
        videoFiles = videoFiles.filter(f => {
            ...
            const MAX_SIZE = 20.0 * 1024 * 1024 * 1024; 
    ...
    if (isMovie) {
        // Return the largest MKV for movies
    ...
    // Check inside batch ranges if no direct match was found
    if (matches.length === 0) {
    ...
    // If looking for Episode 1 and nothing matched, use fallback heuristics
    // to filter out trailers.
    if (epNum === 1) {
